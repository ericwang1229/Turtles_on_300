...
symbols <- toupper(symbols[100:105])
symbols <- c("X601018.SS", "X002081.SZ")
...
#       firstOpen <- .GlobalEnv[[symbol]][[1]]
      reg.time <- timeBasedSeq(paste(initDate, min(index(.GlobalEnv[[symbol]])), sep="/"))
      .GlobalEnv[[symbol]] <- merge(.GlobalEnv[[symbol]], reg.time, fill = 0)
    }
    print(symbol)
  }
...
#         print(paste(CurrentDate, equity, sep = ", equity = "))
        UnitSize = as.numeric(trunc((size * equity)/(x[i-1,'N']*ClosePrice)))
        if ((as.numeric(Lo(x[i-56,])) > 0) && (as.numeric(Vo(x[i,])) > 100))
        {
