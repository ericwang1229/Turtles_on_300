...
for( d in weekdays ) 
  { 
    ...
    print(paste(CurrentDate, ", Equity = ", equity, ", Cash = ", Cash, sep = ""))
    for(symbol in symbols)
    {
        x = get(symbol)
          ...
          if(i > 57)
	        {
      			...
      			UnitSize_By_Eq = as.numeric(trunc((size * equity)/(x[i-1,'N']*ClosePrice)))
      			UnitSize_By_Cash = as.numeric(trunc((size * Cash/5)/(x[i-1,'N']*ClosePrice)))
      			UnitSize = min(UnitSize_By_Eq, UnitSize_By_Cash)
				print(paste(symbol, N,x[i-1,'N'],ClosePrice, UnitSize, sep = ", "))
	#			print(paste(symbol, 
	#						", i = ", i,
	#						", N = ", as.character(N), 
	#						", ClosePrice = ", Cl(x[i,]),
	...
	#						", equity = ", equity,
	#						", N = ", x[i-1, 'N'],
	#						", UnitSize = ", as.character(UnitSize), 
	#						", by_eq = ", as.character(UnitSize_By_Eq), 
	#						", by_cash = ", as.character(UnitSize_By_Cash), sep = ""))
		#	if ((!is.na(Lo(x[i-57,]))) && (!is.na(Vo(x[i,]))) && (as.numeric(Lo(x[i-56,])) > 0) && (as.numeric(Vo(x[i,])) > 100))
      			if ( as.numeric(Vo(x[i,])) > UnitSize)
      			{
      				...
      				{ 
      					addTxn(Portfolio = portfolio, 
					           ...
					           TxnQty = UnitSize , 
					           TxnFees = as.numeric(x[i-2,'Max55'])*UnitSize*-0.00003, 
					           verbose = verbose)
			  					N = as.numeric(x[i-1,'N'])
      					Cash = Cash - as.numeric(x[i-2,'Max55']) * UnitSize +as.numeric(x[i-2,'Max55'])*UnitSize*-0.00003
      				...
      				else if( Posn > 0 && ( as.numeric(Lo(x[i-1,]))  <  as.numeric(x[i-2,'Min20'])  ) ) 
      				{
      					addTxn(Portfolio = portfolio, 
      							 ...
      							 verbose = verbose)
      					N = as.numeric(x[i-1,'N'])
      					Cash = Cash + as.numeric(x[i-2,'Min20']) * Posn
      					updateStrat(Portfolio = portfolio, 
      							  Symbol = symbol, 
      							  TxnDate = CurrentDate, 
      							  PosUnitsQty = 0, 
      				...
      				else if( Posn > 0  && Units < maxUnits && Hi(x[i-1,]) > ( TxnPrice + N * 0.5 ) ) 
      				{
      					addTxn(Portfolio = portfolio, 
      						 ...
      						 verbose = verbose)
      					N = as.numeric(x[i-1,'N'])
      					Cash = Cash - UnitSize * (TxnPrice + N * 0.5 ) + (TxnPrice + N * 0.5)*UnitSize*-0.00003
      					updateStrat(Portfolio = portfolio, 
                                Symbol = symbol, 
                                TxnDate = CurrentDate, 
                                PosUnitsQty = Units + 1, 
